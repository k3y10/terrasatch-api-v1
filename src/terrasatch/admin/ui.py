...
<section class="login-content"><div class="login-brand"><img src="{escape(SATCHY_ASSET_URL)}" alt="Satchy, the TerraSatch field radio mascot"><div><strong>TERRASATCH</strong><small>ADMIN CONSOLE</small></div></div>
...
def render_dashboard(
    *,
    ...
    device_rows = (
        "".join(device_rows_parts)
        or "<tr><td colspan='5' class='dim'>No Edge devices loaded for this organization.</td></tr>"
    )
    ...
    error_box = (
        f"<div class='terminal-alert error'><span>error:</span> {escape(error_message)}</div>"
        if error_message
        else ""
    )
...
<div class="admin-shell"><aside class="sidebar"><a class="brand" href="/admin"><img src="{escape(SATCHY_ASSET_URL)}" alt=""><span><strong>TERRASATCH</strong><small>ADMIN CONSOLE</small></span></a>
...
def render_one_time_key(token: str, organization: str) -> str:
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#08090b"><title>TerraSatch API Key</title>{_styles()}</head><body class="login-shell"><main class="login-terminal key-terminal"><div class="terminal-chrome"><span></span><span></span><span></span><b>credential issuance — one time output</b></div><section class="login-content"><div class="login-brand"><img src="{escape(SATCHY_ASSET_URL)}" alt=""><div><strong>TERRASATCH</strong><small>ADMIN CONSOLE</small></div></div><p><span class="prompt">ts-admin@terrasatch:~$</span> api-key issue</p><h1>Credential generated</h1><div class="terminal-alert warning"><span>warning:</span> this token is shown only in this response.</div><div class="token-block"><code id="issued-token">{escape(token)}</code><button type="button" onclick="navigator.clipboard.writeText(document.getElementById('issued-token').textContent)">copy token</button></div><p><a class="text-link" href="/admin?organization={_attr(organization)}">← return to operations console</a></p></section></main></body></html>"""
