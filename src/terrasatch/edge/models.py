...
class EdgePairing(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    approved_by_api_key_id: Mapped[UUID | None] = mapped_column(ForeignKey("api_keys.id", ondelete="SET NULL"))
...
class EdgeDevice(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    site_id: Mapped[UUID] = mapped_column(ForeignKey("sites.id", ondelete="RESTRICT"), nullable=False, index=True)
    api_key_id: Mapped[UUID] = mapped_column(ForeignKey("api_keys.id", ondelete="RESTRICT"), nullable=False)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    hostname: Mapped[str | None] = mapped_column(String(255))
    platform: Mapped[str | None] = mapped_column(String(100))
    architecture: Mapped[str | None] = mapped_column(String(100))
    agent_version: Mapped[str | None] = mapped_column(String(64))
    hardware_inventory: Mapped[list[dict[str, object]]] = mapped_column(JSON, default=list, nullable=False)
