...
def api_catalog() -> list[ApiCatalogEntry]:
    ...
    entries = [
        ...
        ("GET", "/api/v1/billing/plans", "public", "Customer-safe TerraSatch subscription plans and entitlements without Stripe price identifiers."),
        ("POST", "/api/v1/billing/checkout", "public", "Create a rate-limited Stripe-hosted trial Checkout session from a server-owned plan definition."),
        ("GET", "/api/v1/billing/checkout/status", "public", "Confirm webhook-driven local provisioning state after a Stripe Checkout redirect."),
        ("GET", "/api/v1/billing/subscription", "bearer_api_key", "Read the authenticated organization's managed subscription state and entitlements."),
        ("POST", "/api/v1/billing/portal", "bearer_api_key", "Create a short-lived Stripe Customer Portal session for the authenticated organization."),
        ("POST", "/api/v1/billing/activate", "public", "Consume one single-use owner activation token and set the initial organization-portal password."),
        ("POST", "/api/v1/billing/stripe/webhook", "stripe_webhook", "Verify and idempotently apply Stripe billing lifecycle events."),
        ...
        ("GET", "/api/v1/edge/config", "bearer_api_key", "Return remote configuration and non-sensitive service entitlement state for the authenticated Edge device."),
        ("GET", "/api/v1/edge/commands", "bearer_api_key", "Poll commands assigned only to the authenticated Edge device and site."),
        ("POST", "/api/v1/edge/commands/{command_id}/ack", "bearer_api_key", "Idempotently acknowledge one device-owned command."),
        ("POST", "/api/v1/edge/commands/{command_id}/result", "bearer_api_key", "Report a simulated or failed result for one acknowledged command."),
        ...
        ("POST", "/api/v1/channels", "bearer_api_key", "Create a logical monitored channel subject to managed-plan capacity."),
        ("GET", "/api/v1/channels/{channel_id}", "bearer_api_key", "Read one logical monitored channel."),
        ("PATCH", "/api/v1/channels/{channel_id}", "bearer_api_key", "Update, reassign, enable, or disable a logical channel."),
        ...
        ("GET", "/admin/satchy", "admin_session", "Inspect Satchy proposals, approvals, and simulated outbound lifecycle."),
        ("POST", "/admin/satchy/actions/{action_id}/approve", "admin_session", "Approve or edit a proposed Satchy response before queueing."),
        ("POST", "/admin/satchy/actions/{action_id}/reject", "admin_session", "Reject a proposed Satchy action without outbound execution."),
...
def common_errors() -> list[ErrorCodeReference]:
    ...
    return [
        ...
        ErrorCodeReference(http_status=409, code="resource_conflict", meaning="A resource conflicts with existing state, subscription state, or a configured registration limit."),
        ...
        ErrorCodeReference(http_status=429, code="rate_limited", meaning="A bounded public endpoint exceeded its request threshold; retry later."),
        ErrorCodeReference(http_status=503, code="provider_unavailable", meaning="A required dependency or configured provider is not ready."),
    ]
