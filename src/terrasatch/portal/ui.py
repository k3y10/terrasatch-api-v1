...
from terrasatch.portal.access_ui import ACCESS_STYLES, render_workspace_access
...
def _billing_panel(
    *,
    ...
    plan_code = str(billing.get("plan_code") or "TerraSatch")
    plan = escape({"field": "Individual"}.get(plan_code, plan_code).upper())
    raw_status = str(billing.get("status") or "unknown")
    billing_status = escape({"trialing": "Beta trial", "past_due": "Payment overdue"}.get(raw_status, raw_status).upper())
    ...
    date_message = ""
    if trial_end and billing.get("status") == "trialing":
...
def render_portal(
    *,
    ...
    access_html = render_workspace_access(
        billing, device_count=len(devices), site_count=len(sites),
        online_count=summary.get("online", 0), can_manage=billing_manage_allowed,
    )
