...
from .models import IntegrationConnection, IntegrationCredential, IntegrationOAuthState, IntegrationStatus
...
def workspace_return_url(settings: Settings, *, connection: IntegrationConnection | None, success: bool, reason: str | None = None) -> str:
...
async def begin_authorization(session: AsyncSession, settings: Settings, *, organization_id: UUID, user_id: UUID, role: MembershipRole, connection_id: UUID) -> tuple[IntegrationConnection, str, datetime]:
    connection = await get_connection_for_management(session, organization_id=organization_id, user_id=user_id, role=role, connection_id=connection_id, for_update=True)
    ...
    session.add(IntegrationOAuthState(organization_id=organization_id, connection_id=connection.id, user_id=user_id, provider=connection.provider, state_hash=_state_hash(state), expires_at=expires_at))
...
async def _credential_for_connection(session: AsyncSession, *, organization_id: UUID, connection_id: UUID) -> IntegrationCredential | None:
    return await session.scalar(select(IntegrationCredential).where(IntegrationCredential.organization_id == organization_id, IntegrationCredential.connection_id == connection_id))


async def _save_credentials(session: AsyncSession, settings: Settings, *, connection: IntegrationConnection, payload: dict[str, object]) -> IntegrationCredential:
    credential = await _credential_for_connection(session, organization_id=connection.organization_id, connection_id=connection.id)
    if credential is None:
        credential = IntegrationCredential(organization_id=connection.organization_id, connection_id=connection.id, key_id=settings.integration_encryption_key_id, encrypted_payload="")
...
async def complete_authorization(session: AsyncSession, settings: Settings, *, provider: str, state: str, code: str | None, provider_error: str | None) -> tuple[IntegrationConnection, bool, str | None]:
    ...
    if provider_error:
        ...
        connection.last_error = f"Provider authorization was not completed ({reason})"
        await session.flush()
    ...
    try:
        ...
        await _save_credentials(session, settings, connection=connection, payload=result.credentials)
...
async def active_credentials(session: AsyncSession, settings: Settings, *, connection: IntegrationConnection) -> tuple[dict[str, object], IntegrationCredential]:
    credential = await _credential_for_connection(session, organization_id=connection.organization_id, connection_id=connection.id)
    if credential is None:
        raise ProviderUnavailable("Integration credentials are unavailable; reconnect the provider")
...
async def probe_connection(session: AsyncSession, settings: Settings, *, organization_id: UUID, user_id: UUID, role: MembershipRole, connection_id: UUID) -> IntegrationConnection:
    connection = await get_connection_for_management(session, organization_id=organization_id, user_id=user_id, role=role, connection_id=connection_id, for_update=True)
    ...
    try:
        credentials, _ = await active_credentials(session, settings, connection=connection)
        label, account_id = await get_adapter(connection.provider, settings).probe(credentials)
    except TerraSatchError as error:
        connection.status = IntegrationStatus.ERROR.value
        connection.last_error = error.message[:1000]
        await session.flush()
...
async def disconnect_connection(session: AsyncSession, settings: Settings, *, organization_id: UUID, user_id: UUID, role: MembershipRole, connection_id: UUID) -> IntegrationConnection:
    connection = await get_connection_for_management(session, organization_id=organization_id, user_id=user_id, role=role, connection_id=connection_id, for_update=True)
    credential = await _credential_for_connection(session, organization_id=organization_id, connection_id=connection.id)
