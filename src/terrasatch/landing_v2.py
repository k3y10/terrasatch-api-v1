"""Orange TerraListen public console with live listening and provider health states."""
# ruff: noqa: E501
...
<meta name="description" content="TerraListen provider-aware radio intelligence and Satchy AI-channel control plane.">
...
<aside class="rail">
<section class="card"><div class="title"><span>SYSTEM STATUS</span><b>LIVE</b></div><div class="rows"><div class="row"><span>API</span><b id="api-state" class="health unknown">CHECKING</b></div><div class="row"><span>Database</span><b id="db-state" class="health unknown">CHECKING</b></div><div class="row"><span>Redis</span><b id="redis-state" class="health unknown">CHECKING</b></div></div></section>
<section class="card"><div class="title"><span>REGISTERED EDGE</span><b id="fleet-badge">PRIVATE</b></div><div class="rows"><div class="row"><span>Fleet</span><b id="fleet-summary" class="health unknown">ADMIN SIGN-IN</b></div><div class="row"><span>Device</span><b id="fleet-device">—</b></div><div class="row"><span>Hardware</span><b id="fleet-hardware">—</b></div><div class="row"><span>RX</span><b id="fleet-rx">—</b></div><div class="row"><span>TX</span><b id="fleet-tx">—</b></div><div class="row"><span>Mode</span><b id="fleet-mode">—</b></div></div><div class="fleet-detail" id="fleet-detail">Registered device state is session-protected. Sign into Admin to show heartbeat age, reported hardware, RX/TX capabilities and current Satchy policy.</div></section>
<section class="card"><div class="title"><span>CURRENT COMPATIBILITY</span><b>EDGE</b></div><div class="rows"><div class="row"><span class="compat">RTL-SDR / Nooelec <em>audio capture</em></span><b class="health healthy">RX READY</b></div><div class="row"><span class="compat">HackRF <em>hardware discovery</em></span><b class="health degraded">ADAPTER PENDING</b></div><div class="row"><span class="compat">TX-capable provider <em>plugin/gateway</em></span><b class="health degraded">PROVIDER-GATED</b></div></div><div class="note"><strong>Compatibility:</strong> Nooelec/RTL-SDR is the current receive-ready path. HackRF discovery does not imply TerraListen TX support.</div></section>
<section class="card"><div class="title"><span>SYSTEM INFO</span><b>PROD</b></div><div class="rows"><div class="row"><span>Environment</span><b>__ENV__</b></div><div class="row"><span>Deployment</span><b>__DEPLOY__</b></div><div class="row"><span>Version</span><b>v__VERSION__</b></div><div class="row"><span>Revision</span><b id="revision">—</b></div></div></section>
</aside></main>
<footer><span><b>TERRASATCH</b> · LISTEN. WATCH. LEARN. ADAPT.</span><span class="center">SATCHY AI CHANNEL · LOGICAL FIRST · PROVIDER BOUND</span><span class="policy">TX REQUIRES CAPABLE PROVIDER + OPERATOR POLICY</span></footer></div>
<script>
(()=>{
 const q=id=>document.getElementById(id);
 const normalize=value=>String(value||'unknown').toLowerCase();
 const healthClass=value=>{const s=normalize(value);if(['healthy','pass','online'].includes(s))return 'healthy';if(['degraded','stale','warning'].includes(s))return 'degraded';if(['unhealthy','error','offline','failed'].includes(s))return 'unhealthy';return 'unknown'};
 const paint=(id,value,label)=>{const el=q(id);if(!el)return;const cls=healthClass(value);el.className=`health ${cls}`;el.textContent=label||String(value||'unknown').toUpperCase()};
 const setSignal=(device)=>{const signal=q('radio-signal'),state=q('listen-state'),label=q('listen-label'),source=q('signal-source');if(!signal||!state||!label||!source)return;if(!device){signal.dataset.edge='standby';state.textContent='STANDBY';state.className='orange';label.textContent='LISTENING · WAITING FOR EDGE';source.textContent='STANDBY HEARTBEAT';return}signal.dataset.edge=device.health||'standby';const online=device.health==='online';const stale=device.health==='stale';state.textContent=online?'EDGE ONLINE':stale?'EDGE STALE':String(device.health||'standby').toUpperCase();state.className=online?'green':stale?'yellow':device.health==='offline'?'red':'orange';label.textContent=online?'LISTENING · EDGE HEARTBEAT LIVE':stale?'LISTENING · HEARTBEAT STALE':'LISTENING · WAITING FOR EDGE';source.textContent=device.name||'REGISTERED EDGE'};
 async function updateHealth(){try{const r=await fetch('/health/ready',{cache:'no-store'}),p=await r.json(),ok=r.ok&&p.status==='healthy';const top=q('system-state');top.textContent=ok?'SYSTEMS ONLINE':'SYSTEM DEGRADED';top.className=`state ${ok?'healthy':'unhealthy'}`;paint('api-state',ok?'healthy':'unhealthy',ok?'HEALTHY':'DEGRADED');q('revision').textContent=p.revision||'—';const d=Object.fromEntries((p.dependencies||[]).map(x=>[x.name,x.status]));paint('db-state',d.database||'unknown');paint('redis-state',d.redis||'unknown')}catch(_){const top=q('system-state');top.textContent='UNAVAILABLE';top.className='state unhealthy';paint('api-state','unhealthy','UNKNOWN');paint('db-state','unknown');paint('redis-state','unknown')}}
 async function updateFleet(){try{const r=await fetch('/admin/fleet-status',{cache:'no-store',credentials:'same-origin'});if(!r.ok)return;const p=await r.json(),devices=p.devices||[],summary=p.summary||{};q('fleet-badge').textContent='LIVE';q('fleet-summary').textContent=`${summary.online||0} ONLINE / ${summary.total||0} REGISTERED`;q('fleet-summary').className=`health ${summary.online?'healthy':summary.stale?'degraded':summary.total?'unhealthy':'unknown'}`;const device=devices.find(x=>x.health==='online')||devices.find(x=>x.health==='stale')||devices[0];if(!device){q('fleet-device').textContent='No registered Edge';q('fleet-detail').textContent='Pair an Edge device from Admin to begin heartbeat and capability reporting.';setSignal(null);return}q('fleet-device').textContent=device.name||'Edge';q('fleet-hardware').textContent=device.hardware||'No hardware reported';q('fleet-rx').textContent=device.rx_supported?(device.rx_enabled?'SUPPORTED · ON':'SUPPORTED · OFF'):'UNAVAILABLE';q('fleet-rx').className=device.rx_supported?'green':'';q('fleet-tx').textContent=device.tx_supported?(device.tx_enabled?'SUPPORTED · ARMED':'SUPPORTED · OFF'):'UNAVAILABLE';q('fleet-tx').className=device.tx_supported?(device.tx_enabled?'yellow':'blue'):'';q('fleet-mode').textContent=device.mode||'IDLE';const age=device.age_seconds==null?'never reported':device.age_seconds<60?`${device.age_seconds}s ago`:device.age_seconds<3600?`${Math.floor(device.age_seconds/60)}m ago`:`${Math.floor(device.age_seconds/3600)}h ago`;q('fleet-detail').innerHTML=`<strong>${String(device.health||'unknown').toUpperCase()}</strong> · last heartbeat ${age} · ${device.capabilities?.length||0} reported capabilities`;setSignal(device)}catch(_){}}
 updateHealth();updateFleet();setInterval(updateHealth,15000);setInterval(updateFleet,10000);
    ...
    replacements = {
        "__ENV__": escape(environment.upper()),
        "__DEPLOY__": escape(deployment),
