...
def render_data_sources(
    *,
    ...
    if selected_organization:
        create_panel = f'''<section><div class="head"><div><small>// REGISTER SOURCE</small><h2>Add a disabled-by-default adapter</h2></div></div><form class="grid" method="post" action="/admin/data-sources"><input type="hidden" name="organization" value="{_attr(selected_organization)}"><input type="hidden" name="csrf_token" value="{_attr(csrf_token)}"><label>Name<input name="name" placeholder="Snowbird flaik" required></label><label>Provider<input name="provider" placeholder="flaik" required></label><label>Source kind<input name="source_kind" placeholder="operational_context" required></label><label>Adapter<select name="adapter_key"><option value="manual_snapshot">manual_snapshot</option><option value="flaik_context">flaik_context</option></select></label><label>Endpoint URL<input name="endpoint_url" placeholder="https://authorized-provider-api"></label><label>Credential reference<input name="credential_reference" placeholder="env://SNOWBIRD_FLAIK_CLIENT_SECRET (never a raw token)"></label><label class="wide">Configuration JSON<textarea name="configuration_json" rows="7" placeholder='{{"mode":"fixture","resort_name":"Snowbird"}}'>{{}}</textarea></label><label class="check"><input type="checkbox" name="enabled" value="true"> Enable immediately</label><button>Create source</button></form></section>'''
